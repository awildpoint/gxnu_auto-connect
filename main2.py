...
class CampusNetworkGUI:
    ...
    def create_status_tab(self, parent):
        ...
        # 新增：退出程序按钮
        exit_button = ttk.Button(control_frame, text="退出程序", 
    ...
    def create_log_tab(self, parent):
        ...
        self.log_text = scrolledtext.ScrolledText(log_display, height=15, width=70)
    ...
    def minimize_to_tray(self):
        ...
        if not self.tray_icon_created:
            ...
            # 如果托盘图标已存在，确保它正在运行
            if hasattr(self.tray_icon, '_thread') and not self.tray_icon._thread.is_alive():
                threading.Thread(target=self.tray_icon.run, daemon=True).start()
    ...
    def run(self):
        ...
        def on_closing():
            """窗口关闭事件处理"""
            if messagebox.askyesno("确认", "确定要最小化到托盘吗？\n选择'否'将退出程序"):
